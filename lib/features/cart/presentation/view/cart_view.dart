...
//     final totalValueStyle = totalLabelStyle.copyWith(color: AppTheme.primaryColor);
...
class _CartViewState extends State<CartView> with TickerProviderStateMixin {
  ...
  void initState() {
    super.initState();
    context.read<CartViewModel>().add(LoadCart());
    _buttonController = AnimationController(vsync: this, duration: const Duration(seconds: 1))..repeat(reverse: true);
  }
  ...
  Widget _buildEmptyCart(BuildContext context) {
    return Center(
      child: Column(
        mainAxisAlignment: MainAxisAlignment.center,
        children: [
          Lottie.asset('assets/animations/shopping cart.json', width: 250),
          const SizedBox(height: 16),
          const Text('Your cart is empty', style: AppTheme.subheadingStyle),
          const SizedBox(height: 8),
          const Text('Add items to get started', style: AppTheme.captionStyle),
          ...
          AnimatedBuilder(
            ...
            builder: (context, child) {
              return Transform.scale(scale: 1 + (_buttonController.value * 0.05), child: child);
            },
            child: ElevatedButton(
              onPressed: () {
