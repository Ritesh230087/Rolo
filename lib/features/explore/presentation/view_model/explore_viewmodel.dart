...
class ExploreViewModel extends Bloc<ExploreEvent, ExploreState> {
  final GetExploreDataUsecase _getExploreDataUsecase;

  ExploreViewModel({
    required GetExploreDataUsecase getExploreDataUsecase,
  })  : _getExploreDataUsecase = getExploreDataUsecase,
        super(const ExploreState()) {
    on<LoadExploreData>(_onLoadExploreData);
    on<SearchQueryChanged>(_onSearchQueryChanged);
    on<CategoryFilterChanged>(_onCategoryFilterChanged);
  ...
  Future<void> _onLoadExploreData(
    ...
    // Show loading UI only if it's the initial load, not a background refresh
    if (!event.isRefresh) {
      emit(state.copyWith(status: ExploreStatus.loading));
    }

    final result = await _getExploreDataUsecase();

    result.fold(
      (failure) => emit(state.copyWith(status: ExploreStatus.failure, errorMessage: failure.message)),
      (data) {
        final (products, categories) = data;
        // Update the master lists of products and categories
        emit(state.copyWith(
          status: ExploreStatus.success,
          allProducts: products,
          categories: categories,
        ));
        // After loading, immediately apply any existing filters (like search query)
        _applyFilters(emit);
      },
    );
  }
  ...
  // This private method contains all the filtering and sorting logic
  void _applyFilters(Emitter<ExploreState> emit) {
    // Start with the master list of all products
    List<ProductEntity> filteredList = List.from(state.allProducts);
    ...
    // Emit the final state with the newly filtered and sorted list
    emit(state.copyWith(filteredProducts: filteredList, status: ExploreStatus.success));
  }
}